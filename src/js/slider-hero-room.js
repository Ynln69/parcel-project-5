...
function moveSlideNext(n) {
  if (n === 0) {
    ...
    Frame.run(slides[slides.length - 1], { left: '-100%', duration: 0.5 });
    Frame.run(slides[n], { left: '0px', duration: 0.5 });
  } else {
    slides[n].style.left = '100%';
    slides[n - 1].style.left = '0%';
    Frame.run(slides[n - 1], { left: '-100%', duration: 0.5 });
    Frame.run(slides[n], { left: '0px', duration: 0.5 });
  }
}
...
function moveSlidePrev(n) {
  if (n === 0) {
    ...
    Frame.run(slides[0], { left: '100%', duration: 0.5 });
    Frame.run(slides[n], { left: '0px', duration: 0.5 });
  } else {
    slides[n].style.left = '-100%';
    slides[n + 1].style.left = '0px';
